  ...
  useEffect(() => {
    ...
    const carregarDados = async () => {
      try {
        ...
        if (!id) {
          if (mounted) {
            setErro("ID do artigo não fornecido");
            setLoading(false);
          }
          return;
        }

        // Buscar usando list e find
        const artigos = await base44.entities.ArtigoBlog.list();
    ...
    return (
      <div className="min-h-screen bg-gray-50 py-8 flex items-center justify-center">
        <Card className="p-12 text-center max-w-md mx-4">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">{erro || "Artigo não encontrado"}</h3>
          <p className="text-gray-600 mb-6">Este artigo não existe ou foi removido.</p>
          <Button onClick={() => navigate(createPageUrl("Blog"))} className="bg-gradient-to-r from-pink-600 to-rose-600">
            Voltar para o Blog
          </Button>
        </Card>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white py-8">
      <div className="max-w-4xl mx-auto px-4">
        <Button variant="ghost" onClick={() => navigate(createPageUrl("Blog"))} className="mb-6">
        ...
        <Card className="border-none shadow-2xl overflow-hidden">
          <div className="h-48 md:h-96 bg-gradient-to-br from-pink-100 to-rose-100 flex items-center justify-center text-6xl md:text-8xl">
            {artigo.imagem_capa ? <img src={artigo.imagem_capa} alt={artigo.titulo} className="w-full h-full object-cover" /> : "✨"}
          </div>
          ...
          <CardContent className="p-6 md:p-12">
            <div className="flex flex-wrap items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-500">
                <span className="flex items-center gap-1"><Calendar className="w-4 h-4" />{format(new Date(artigo.created_date), "dd/MM/yyyy")}</span>
                <span className="flex items-center gap-1"><Clock className="w-4 h-4" />{artigo.tempo_leitura} min</span>
              </div>
              <div className="flex items-center gap-2 ml-auto">
                <Button variant="outline" size="sm" onClick={handleCurtir} disabled={!user} className={artigo.curtidas?.includes(user?.email) ? "text-red-600 border-red-600" : ""}>
                  <Heart className={`w-4 h-4 mr-2 ${artigo.curtidas?.includes(user?.email) ? 'fill-red-600' : ''}`} />
                  {artigo.total_curtidas || 0}
                </Button>
                <Button variant="outline" size="sm" onClick={() => setMostrarCompartilhar(true)}>
            ...
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-6 leading-tight">{artigo.titulo}</h1>
            <p className="text-lg md:text-xl text-gray-600 mb-8 leading-relaxed border-l-4 border-pink-500 pl-6 italic">{artigo.resumo}</p>

            <div className="prose prose-lg max-w-none">
              {artigo.conteudo.split('\n\n').map((paragrafo, index) => (
                <p key={index} className="text-gray-700 mb-6 leading-relaxed text-base md:text-lg">{paragrafo}</p>
              ))}
            </div>
            ...
            <div className="mt-12 pt-8 border-t">
              ...
              <div className="mb-8">
                <Textarea
                  ...
                  rows={4}
                />
                <Button onClick={handleComentar} disabled={!user} className="bg-pink-600 hover:bg-pink-700">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-gray-500">{format(new Date(comentario.created_date), "dd/MM/yyyy 'às' HH:mm")}</span>
                        </div>
        ...
        <Dialog open={mostrarCompartilhar} onOpenChange={setMostrarCompartilhar}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Escolha onde você quer compartilhar</DialogDescription>
            </DialogHeader>
            <div className="grid gap-3">
              <Button onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(artigo.titulo + '\n\n' + window.location.href)}`, '_blank')} className="w-full bg-green-600 hover:bg-green-700">
                📱 WhatsApp
              </Button>
              <Button onClick={() => window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`, '_blank')} variant="outline">
                📘 Facebook
              </Button>
            </div>
          </DialogContent>
