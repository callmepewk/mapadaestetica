...
export default function TermosCondicoes() {
  ...
  useEffect(() => {
    ...
    if (!termosAceitos || termosAceitos !== 'true') {
      // Delay para mostrar a barra após 1 segundo
      setTimeout(() => {
  ...
  return (
    <>
      {/* Barra de Cookies/Termos - Rodapé Fixo */}
      <AnimatePresence>
        {mostrarBarra && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-r from-gray-900 to-gray-800 border-t-2 border-[#F7D426] shadow-2xl"
          >
            <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="flex items-center gap-3 text-white text-sm flex-1">
                <FileText className="w-5 h-5 text-[#F7D426] flex-shrink-0" />
                <p className="text-center sm:text-left">
                  Utilizamos cookies e dados para melhorar sua experiência. 
                  <button 
                    onClick={() => setDialogAberto(true)}
                    className="ml-1 underline hover:text-[#F7D426] transition-colors font-semibold"
                  >
      ...
      <Dialog open={dialogAberto} onOpenChange={setDialogAberto}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4 text-sm">
            ...
            <section>
              <h3 className="font-semibold mb-2">8. Cookies e Tecnologias</h3>
              <p className="text-gray-600">
