...
export default function TrendHeatmap({ matrix }) {
  ...
  const safe = Array.isArray(matrix) && matrix.length ? matrix : base;
  const normalized = safe.map((row = []) =>
    Array.from({ length: 24 }, (_, i) => Math.max(0, Number(row[i] || 0)))
  );
  const max = Math.max(1, ...normalized.flat());
  const color = (v) => {
    const n = v / max; // 0..1
